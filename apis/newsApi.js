...
newsApp.get('/getnews',expressAsyncHandler(async(request,response)=>{
    let newsResponse=await newsApiApp.v2.everything({q:'india',language:'en',sortBy:'publishedAt'});
    let articles=newsResponse.articles;
    response.send({payload:articles})
}))
